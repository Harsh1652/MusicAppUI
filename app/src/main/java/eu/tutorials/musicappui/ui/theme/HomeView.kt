...
@Composable
fun Home(){
    val categories = listOf("Hits", "Happy", "Workout", "Running", "Yoga",  "Sad", "Romantic")
    val grouped = listOf<String>("New Release", "Favorites", "Top Rated").groupBy { it[0] }
    LazyColumn {
        grouped.forEach{
            stickyHeader { 
                Text(text = it.value[0], modifier = androidx.compose.ui.Modifier.padding(16.dp))
                LazyRow {
                    items(categories){
                        cat ->
...
fun BrowserItem(cat: String, drawable: Int){
    Card(modifier = androidx.compose.ui.Modifier.padding(16.dp).size(200.dp),
